...
# 新規: ベクトルストアと埋め込み生成のためのモジュールをインポート
...
# プロンプトテンプレートの作成
# 変更点: contextプレースホルダーの追加
system_template = (
    "以下のcontextを参照して、簡潔で正確な回答を提供してください: {context}"
)
...
# 新規: Chromaベースのベクトルストアを使用した外部知識検索機能の追加
vectorstore = Chroma(
...
async def get_chat_response(question: str, session_id: str = "default"):
    ...
    # 変更点: contextを含めてchain_with_historyを呼び出し回答を取得
